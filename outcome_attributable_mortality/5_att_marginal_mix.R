...
# helper: susceptible / resistant counts
...
# Calculate AR from mixed-effects logistic models using df_list
results_list <- list()
k <- 1
...
for (i in seq_along(pathogens)) {
  ...
  if ("siteid" %in% names(df_used)) {
    df_used$siteid <- as.factor(df_used$siteid)
  }
  ...
  fml_overall <- as.formula(
    paste0("first28_death ~ ", var_name, " + (1 | siteid)")
  )
  ...
  if ("infection_types" %in% names(df_used)) {
    ...
    fml_inf <- as.formula(
      paste0("first28_death ~ infection_types * ", var_name, " + (1 | siteid)")
    )
  ...
  if ("age_group_new" %in% names(df_used)) {
    ...
    fml_age <- as.formula(
      paste0("first28_death ~ age_group_new * ", var_name, " + (1 | siteid)")
    )
  ...
  if ("country_income" %in% names(df_used)) {
    ...
    fml_inc <- as.formula(
      paste0("first28_death ~ country_income * ", var_name, " + (1 | siteid)")
    )
...
for (var_name in pathogens) {
  ...
  pop2 <- pop %>%
    mutate(
      resistant = grepl("-resistant$", as.character(.data[[var_name]]), ignore.case = TRUE)
    )
...
# Calculate subgroup-specific Ipop from pop
ipop_list <- list()
...
# Save
saveRDS(final_df, "data/att28_paf_marginal_mix.RData")
