...
# Load data
df_site <- readRDS("data/clean data/data_table_index_new.RData")
df_ast <- readRDS("data/clean data/ast_all_index.RData")
...
df_used_crp <- process_data(df, "Pseudomonas spp.", "ris_Carbapenem")
df_used_vre <- process_data(df, "Enterococcus spp.", "Vancomycin")
df_used_crsa <- process_data(df, "S. aureus", "Methicillin")

# Combine all lists into one
df_used <- c(df_used_crp, df_used_vre, df_used_crsa)
...
label_list <- list(
  grid.text("CRP", gp = gpar(fontsize = 32, fontface = "bold", fontfamily = "Times New Roman")),
  grid.text("VRE", gp = gpar(fontsize = 32, fontface = "bold", fontfamily = "Times New Roman")),
  grid.text("MRSA", gp = gpar(fontsize = 32, fontface = "bold", fontfamily = "Times New Roman"))
)

# Combine the labels with the plot list
combined_plot <- wrap_plots(
  wrap_plots(label_list[[1]], p_list[[1]], p_list[[2]], p_list[[3]], ncol = 4, widths = c(0.15, 1, 1, 1)),
  wrap_plots(label_list[[2]], p_list[[4]], p_list[[5]], p_list[[6]], ncol = 4, widths = c(0.15, 1, 1, 1)),
  wrap_plots(label_list[[3]], p_list[[7]], p_list[[8]], p_list[[9]], ncol = 4, widths = c(0.15, 1, 1, 1)),
  ncol = 1
) +
...
# Save
CairoPDF(file = "output/figure/proportion_resist_high3_h.pdf", 
         width = 27, height = 21)
plot(final_plot)
dev.off()
###