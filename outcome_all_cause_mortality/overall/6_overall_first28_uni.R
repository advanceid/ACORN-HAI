...
univariate_model <- survreg(Surv(time, event) ~ ent_car, data = df, dist = "lognormal")
...
rows <- list(
  ...
  c("Hospital-acquired BSI", ratio_df[16, c(5:8, 1)]),
  c("Healthcare-associated BSI", ratio_df[17, c(5:8, 1)])
 )
...
saveRDS(result_save, "data/frist28_table_univariable_overall.RData")
