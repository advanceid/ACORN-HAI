...
# Specific antibiotics for Polymyxin
polymyxins_spec_anti_cols <- which(colnames(ast_used) %in% anti$anti_names[anti$anti_group == "Polymyxin"])
...
car_anti_cols <- which(colnames(all_ast_merged) %in% c(anti$anti_names[anti$anti_group == "Carbapenem"], "ris_Carbapenem"))

tgc_anti_cols <- which(colnames(all_ast_merged) %in% c(anti$anti_names[anti$anti_group == "Third-generation cephalosporin"], "ris_Third-generation cephalosporin"))
...
df_baseline <- matrix(NA, nrow = nrow(data[[1]]), ncol = 23)
colnames(df_baseline) <- c("recordid", "date_enrolment",
                           "age_raw", "age_new", "age_group", "age_group_new",
                           "age_display", "age_unit",
                           "age_display_new", "age_unit_new",
                           "country_ab", "country", 
...
# 
# New age group (GRAM)
# <1 year, 1–4 years, 5–14 years, 15–49 years, 50–69 years, and ≥70 years
df_baseline[,"age_group_new"] <- cut(
  numeric_vector,
  breaks = c(-Inf, 1, 5, 15, 50, 70, Inf),
  right = FALSE,
  labels = c(
    "<1 year",
    "1–4 years",
...
df_baseline[is.na(df_baseline[, "age_group"]), "age_group_new"] <- NA

# Age display new for six groups
for (i in 1:nrow(data[[1]])) {
  grp <- df_baseline[i, "age_group_new"]
  
  if (!is.na(grp)) {
    if (grp %in% c(2:6)) {
      df_baseline[i, "age_display_new"] <- df_baseline[i, "age_new"]
      df_baseline[i, "age_unit_new"]    <- "years"
      
    } else if (grp == 1) {
      if (!is.na(data[[1]]$brthdtc[i]) && !is.na(data[[1]]$dmdtc[i])) {
        df_baseline[i, "age_display_new"] <- as.numeric(interval(data[[1]]$brthdtc[i],
                                                             data[[1]]$dmdtc[i]) / days(30))
      } else {
        df_baseline[i, "age_display_new"] <- data[[1]]$aged[i]/30
      }
      df_baseline[i, "age_unit_new"] <- "months"
    }
  }
}
...
# The hierarchical comorbidities: diabetes, liver disease and cancer/tumor should not haave 1 for both.
cmb_comorbidities$cmb_comorbidities___mld<-ifelse(cmb_comorbidities$cmb_comorbidities___liv==1, 0, cmb_comorbidities$cmb_comorbidities___mld)
cmb_comorbidities$cmb_comorbidities___onc<-ifelse(cmb_comorbidities$cmb_comorbidities___mst==1, 0, cmb_comorbidities$cmb_comorbidities___onc)
cmb_comorbidities$cmb_comorbidities___diab<-ifelse(cmb_comorbidities$cmb_comorbidities___diad==1, 0, cmb_comorbidities$cmb_comorbidities___diab)

system.time({
  for (i in 1:nrow(cmb_comorbidities)) {
    if (cmb_comorbidities$cmb_comorbidities___none[i] == 1) {
      
      df_baseline[i,"comorbidities_CCI"] <- 0
      
    } else if (cmb_comorbidities$cmb_comorbidities___none[i] == 0) {

      # This is the modified CCI Score.
      # See, doi: 10.5152/TurkThoracJ.2022.21076
      
      df_baseline[i,"comorbidities_CCI"] <- 
        2*cmb_comorbidities$cmb_comorbidities___cog[i] +   
        2*cmb_comorbidities$cmb_comorbidities___dem[i] +   
        cmb_comorbidities$cmb_comorbidities___cpd[i] + 
        cmb_comorbidities$cmb_comorbidities___rheu[i] + 
        0*cmb_comorbidities$cmb_comorbidities___pep[i] + 
        2*cmb_comorbidities$cmb_comorbidities___mld[i] + #linked with liv-shouldn't have both?
        0*cmb_comorbidities$cmb_comorbidities___diab[i] + #linked with diad
        cmb_comorbidities$cmb_comorbidities___diad[i] +  #linked with diab
        2*cmb_comorbidities$cmb_comorbidities___hop[i] +
        cmb_comorbidities$cmb_comorbidities___renal[i] + 
        2*cmb_comorbidities$cmb_comorbidities___onc[i] + #linked with mst-shouldn't have both?
        4*cmb_comorbidities$cmb_comorbidities___liv[i] + #linked with mld-shouldn't have both?
        6*cmb_comorbidities$cmb_comorbidities___mst[i] + #linked with onc-shouldn't have both?
        4*cmb_comorbidities$cmb_comorbidities___aids[i]+ 
        0*cmb_comorbidities$cmb_comorbidities___oth[i]
...
strict_vap <- data[[6]] %>%
  ...
  select(
    recordid,
    temp_cxr, white_bc,
    new_onset_ps, new_onset_wt, rales, wor_ge,
    temp_ins, leu,
    ap_tac, whe_ral, cough, wor_ge_1,
    fev_hyp, leu_1,
    new_onset_ps_1, new_onset_worc, ral_bron, wor_ge_2
  ) %>%
  mutate(
    group1 = (
      (temp_cxr == "Y" | white_bc == "Y") &
        (new_onset_ps == "Y" | new_onset_wt == "Y" | rales == "Y" | wor_ge == "Y")
    ),
    
    group2 = (
      (temp_ins == "Y" | leu == "Y") &
        (ap_tac == "Y" | whe_ral == "Y" | cough == "Y" | wor_ge_1 == "Y")
    ),
    
    group3 = (
      (fev_hyp == "Y" | leu_1 == "Y") &
        (new_onset_ps_1 == "Y" | new_onset_worc == "Y" | ral_bron == "Y" | wor_ge_2 == "Y")
    ),
    
    strict_vap = if_else(group1 | group2 | group3, "Y", "N")
  )

## respiration
# Combine
# Sofa_res_3=Sofa_res_1/sofa_res_2_3
# If Sofa_res_3>400 => Sofa_score_1_1=0; Sofa_res_3<400 => Sofa_score_1_1=1; Sofa_res_3<300 => Sofa_score_1_1=2; Sofa_res_3<200 => Sofa_score_1_1=3; Sofa_res_3<100 => Sofa_score_1_1=4
# Currently if Sofa_res_1=0 and sofa_res_2_3/=0 then Sofa_res_3=0 and sofa_score_1_1=0
sofa_all$sofa_res_1<-ifelse(sofa_all$sofa_res_1==0, NA, sofa_all$sofa_res_1)
sofa_all$sofa_score_1_1<-ifelse(is.na(sofa_all$sofa_res_1), NA, sofa_all$sofa_score_1_1)
...
# 
severity_new <- matrix(NA, nrow = nrow(severity), ncol = 6)
colnames(severity_new) <- c("recordid", "adm_ward_types_ori", "adm_ward_types_new",
                            "severity_score", "severity_score_scale", "hai_surg")
#
...
# Surgery from the admission until the day of HAI infection onset
severity_new[, "hai_surg"] <- sapply(severity$hai_surg, convert_to_binary)
...
# "mental_qpitt"
#Question, For qpitt: Altered mental status, how does this relate to our data?
#Question, 1=Alert, 2=disorientated, 3=stuporous, 4=comatose, 5=unknown
#From doi: 10.1177/20499361241231147, Glasgow Coma Scale (GCS) score < 15

condition_mental_qpitt <- pitt_raw$men_sta %in% c(2:4)
condition_mental_qpitt_other <- pitt_raw$men_sta %in% c(1, 5)
...
bsi_excl_org <- c("Salmonella", "Listeria",
                  "Cryptococcus", "Aspergillus", "Trichosporon", "Bipolaris", "Geotrichum", "Unspecified fungus")
...
# anti_group_ent (update anti_group)
...
# Save
file_paths <- c(
  "data/clean_data/infection_types_index.RData",
  "data/clean_data/baseline_outcomes_index.RData",
  "data/clean_data/all_vap_bsi.RData",
  "data/clean_data/vap_bsi_index.RData",
  "data/clean_data/ast_all.RData",
  "data/clean_data/ast_all_index.RData",
  "data/clean_data/anti_treat_index.RData",
  "data/clean_data/anti_treat_index_poly.RData",
  "data/clean_data/episode_for_incidence.RData"
)
...
saveRDS(xx_data, file = "data/clean_data/xinxin_data_latest.rds")
```