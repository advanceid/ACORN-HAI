...
colnames(res) <- c("Characteristics", 
                   ...
                   "Adjusted HR (95%CI)_multi_vap_2", "P value_multi_vap_2",
...
table <- res %>%
  ...
  tab_spanner(
    label = md("**<span style = 'color:black;'>Multivariable analysis</span><sup>[2]</sup>**"),
    columns = 6:7,
    level = 1,
    id = "spanner_multi_vap_2"
  ) %>%
  tab_spanner(
  ...
  tab_spanner(
    label = md("**<span style = 'color:black;'>Healthcare-associated BSI</span><sup>[1]</sup>**"),
    columns = 14:17,
    level = 2,
    id = "spanner_bsi2"
  ) %>%
  ...
  cols_label(
    ...
    `Adjusted HR (95%CI)_multi_vap_2` = md("Adjusted HR (95%CI)"),
  ...
  tab_source_note(
    source_note = html("Note: [1] Carbapenem-resistant <i>Acinetobacter</i> spp. was excluded due to low prevalence (&lt;5%), [2] final model.")
  ) %>%
  tab_source_note(
