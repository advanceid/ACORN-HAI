...
pdf_files <- c("output/pdf/readmission_CIF_overall.pdf",
               "output/pdf/readmission_CIF_sub.pdf")
...
pdf_images_annotated <- mapply(function(image, label) {
  image_annotate(image, label, gravity = "northwest", size = 10, color = "black", location = "+80+20", weight = 400, font = "Times New Roman")
...
# Save
image_write(final_image, path = "output/pdf/readmission_com.pdf", format = "pdf")
###